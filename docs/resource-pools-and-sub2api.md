...
这套模式适合以下场景：同一批官方采购的 Key，需要通过不同 Base URL 同时服务 OpenAI 格式的 Chat/Batch 与 Anthropic 格式的 Coding；Sub2API 只负责下游用户、额度和用量，api-load 是唯一接触真实 Key 的调度层。
...
2. 在池内添加协议端点。例如：
   - `OpenAI`：渠道类型 `openai`，Base URL 填 OpenAI 兼容地址；
   - `Anthropic`：渠道类型 `anthropic`，Base URL 填 Anthropic 地址或其固定路径前缀。
3. 在池内批量粘贴 Key；支持换行或半角逗号分隔。Key 与 URL 分开管理，同一 Key 在池内只保存一次。
4. 创建两个标准分组，例如 `chat` 与 `coding`：
   - `chat` 使用 OpenAI 渠道；
   - `coding` 使用 Anthropic 渠道；
   - 两者绑定同一个资源池；
   - `chat` 选择 OpenAI 端点，`coding` 选择 Anthropic 端点；
   - 分组不再维护自己的 upstream 与官方 Key。
5. 在 Sub2API 中分别把 OpenAI 与 Anthropic 上游指向对应的 api-load 分组：
   - `https://api-load.example.com/proxy/chat`
   - `https://api-load.example.com/proxy/coding`
6. 官方 Key 只保存在 api-load。Sub2API 只保存调用 api-load 所需的下游代理凭据，不能再做真实 Key 轮询、同请求跨上游重试或 Anthropic→OpenAI 格式转换。
...
成功请求会刷新 1 小时滑动 TTL。粘性有利于项目上下文与上游缓存持续命中，但它不是永久锁死：除 404 外，上游调用失败会自动禁用当前共享 Key；普通可重放请求会迁移，并在成功后重绑到另一 Key。分组选择的协议端点在重试期间保持不变。
...
| 上游结果 | api-load 行为 |
| --- | --- |
| 普通 429 | 自动禁用该 Key，使用它的所有协议停止使用；普通可重放请求迁移并在成功后重绑 |
| 明确 quota / credit / billing exhausted，或 402 | 默认把该 Key 标为凭据失效，Chat 与 Coding 都停止使用，需手动测试或启用恢复；池配置了配额自动恢复调度时改为全局冷却到下一个恢复点，到点自动回到轮转 |
| 401 / 403 | 自动禁用该 Key，所有协议停止使用，需管理员恢复 |
| 其他 4xx（404 除外）、网络错误 / 5xx | 自动禁用该 Key；普通可重放请求可以迁移 |
| 404 | 不计失败次数，也不改变资源状态 |
...
OpenAI Batch 和 File 都是上游账户级对象，不能在不同协议端点或官方 Key 之间随意查询：

- File 上传成功后，file ID 会绑定创建它的协议端点与 Key；
- 创建 Batch 时，api-load 根据 `input_file_id` 强制回到 File 的原端点与 Key；
- batch ID 以及响应里的 input/output/error file ID 会持久化绑定；
- 查询、取消、文件元数据与内容下载只访问原端点与 Key；
- 原 Key 不可用时明确返回错误，不会换 Key；
...
拥有 Batch/File 绑定的 Key 或协议端点不能被永久删除，只能先停用；对应对象不再需要后再做数据清理。资源池仍被分组引用时同样禁止删除。

## 运行检查

- api-load 日志里的 `resource_id` 应能确认请求实际落到哪个共享 Key，`upstream_addr` 可确认所用协议端点；
- 相同 `X-Api-Load-Affinity` 的连续 Coding 请求应保持同一 `resource_id`；
- 普通 429 后最多约 2 秒应切换资源，后续请求保持在新资源；
- quota/401/403 后，同一 Key 在 OpenAI 与 Anthropic 两条路由都不再被选择；
