...
成功请求会刷新 1 小时滑动 TTL。粘性有利于项目上下文与上游缓存持续命中，但它不是永久锁死：当前协议通道普通 429 时，api-load 最多等待池配置的 2 秒，然后迁移并永久重绑到另一物理资源。
...
禁用资源会立即停止调度；“恢复”会清除失败计数、全局冷却与停用原因。原始 Key 不会通过管理 API 返回，页面只显示末四位掩码。
