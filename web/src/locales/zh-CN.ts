export default {
  ...
  resourcePools: {
    title: "资源池",
    subtitle: "把官方上游 URL 与 Key 作为不可拆分的物理资源，并供多个协议分组共享调度。",
    routingNote:
      "资源池是唯一的真实 Key 调度层。除 404 外，上游调用失败会自动禁用整个 URL + Key，OpenAI 与 Anthropic 分组会一起停止使用。",
    createPool: "新建资源池",
    createFirst: "创建第一个资源池",
    editPool: "编辑资源池",
    deletePool: "删除资源池",
    deletePoolConfirm: "删除该资源池及其资源？仍被分组或 Batch/File 对象占用时，服务端会拒绝删除。",
    empty: "还没有资源池。先创建一个池，再录入成对的官方 URL 与 Key。",
    name: "名称",
    ...
    seconds: "{value} 秒",
    addResources: "批量添加资源",
    addFirstResource: "添加第一组 URL + Key",
    noResources: "池内还没有物理资源",
    manageResources: "管理资源",
    hideResources: "收起资源",
    searchPlaceholder: "搜索名称、上游 URL，或粘贴完整 Key 精确查找",
    ...
    importHelp:
      "上游地址只需填写一次，下面可以直接粘贴整批 Key；同一地址可以反复追加，新 Key 会添加，已有 URL + Key 会自动跳过。",
    ...
    resourceAppendResult: "已添加 {added} 个新 Key，跳过 {skipped} 个池内已有资源。",
    configImportResult: "已导入 {added} 个新资源，已有资源已自动跳过。",
    secretNote: "原始 Key 只在本次提交时发送；保存后界面只返回掩码，不支持再次查看明文。",
    importResources: "导入资源",
    importConfig: "导入完整配置",
    importConfigHelp:
      "粘贴由资源池导出的 JSONL 或 CSV；会恢复 URL、Key、启用状态、优先级和权重，健康状态与调用统计从零开始。",
    ...
    boundPoolHelp:
      "当前请求由资源池调度。分组原有 upstream 会保留为休眠配置，不参与请求；取消绑定后会自动恢复使用。",
    legacyRoutingHelp:
