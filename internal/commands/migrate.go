...
import (
	...
	db "api-load/internal/db/migrations"
...
func RunMigrateKeys(args []string) {
	...
	migrateCmd.Usage = func() {
		fmt.Println("API-Load Key Migration Tool")
		fmt.Println()
		fmt.Println("Usage:")
		fmt.Println("  Enable encryption: api-load migrate-keys --to new-key")
		fmt.Println("  Disable encryption: api-load migrate-keys --from old-key")
		fmt.Println("  Change key: api-load migrate-keys --from old-key --to new-key")
		fmt.Println()
