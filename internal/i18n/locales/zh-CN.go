...
var MessagesZhCN = map[string]string{
	...
	"validation.aggregate_no_resource_pool":                  "聚合分组不能直接绑定资源池",
	"validation.resource_pool_no_upstreams":                  "绑定资源池的分组不能再配置旧式 upstreams",
	"validation.resource_pool_not_found":                     "资源池 {{.id}} 不存在",
	"validation.upstreams_required_without_resource_pool":    "未绑定资源池时至少需要配置一个 upstream",
