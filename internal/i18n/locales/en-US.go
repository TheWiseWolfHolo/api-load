...
var MessagesEnUS = map[string]string{
	...
	"validation.aggregate_no_resource_pool":                  "Aggregate groups cannot bind directly to a resource pool",
	"validation.resource_pool_no_upstreams":                  "Resource-pool groups cannot define legacy upstreams",
	"validation.resource_pool_not_found":                     "Resource pool {{.id}} was not found",
	"validation.upstreams_required_without_resource_pool":    "At least one upstream is required when no resource pool is bound",
