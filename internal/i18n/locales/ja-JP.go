...
var MessagesJaJP = map[string]string{
	...
	"validation.aggregate_no_resource_pool":                  "集約グループをリソースプールに直接関連付けることはできません",
	"validation.resource_pool_no_upstreams":                  "リソースプールを使用するグループに従来の upstreams は設定できません",
	"validation.resource_pool_not_found":                     "リソースプール {{.id}} が見つかりません",
	"validation.upstreams_required_without_resource_pool":    "リソースプールを使用しない場合は少なくとも1つの upstream が必要です",
